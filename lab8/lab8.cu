...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if(row<dim){
    float sum = 0.0f;
    int original_row_idx = matRowPerm[row];
    int num_of_nonzeros = matRows[row]; // number of nonzeros in this row;
    
    for(int c=0;c<num_of_nonzeros;c++){
      int idx = matColStart[c]+row;
      int col = matCols[idx];
      sum += matData[idx]*vec[col];
    }

    out[original_row_idx] = sum;
  }
...
int main(int argc, char **argv) {
  ...
  maxRowNNZ = hostJDSRows[0]; // hostJDSRows are Number of non-zeros per row
